                ...
                var sec_C = htmlToPdfmake(`<br>
                    ...
                    <label>2. Details of minimum wages paid to workers & employees (For both current & previous year):</label>
                        <table>
                            ...
                            <?php
                                // Limit to the rows from index 3 to 8
                                $limited_t_wageDetails = array_slice($t_wageDetails, 3, 6);
                                $index = 3; // Start index from 4
                                foreach ($limited_t_wageDetails as $rowData) {
                                    echo '<tr>';
                                        $index++; // Increment index for the next rowx
                                        if ($index == 4) {
                                            echo '<th>Male</th>';
                            ...
                            <?php
                                // Limit to the rows from index 3 to 8
                                $limited_t_wageDetails = array_slice($t_wageDetails, 6, 9);
                                $index = 6; // Start index from 4
                                foreach ($limited_t_wageDetails as $rowData) {
                                    echo '<tr>';
                                        $index++; // Increment index for the next rowx
                                        if ($index == 7) {
                            ...
                            <?php
                                // Limit to the rows from index 3 to 8
                                $limited_t_wageDetails = array_slice($t_wageDetails, 9, 12);
                                $index = 9; // Start index from 4
                                foreach ($limited_t_wageDetails as $rowData) {
                                    echo '<tr>';
                                        $index++; // Increment index for the next rowx
                                        if ($index == 10) {
                    ...
                    <label>8.1. Provide details of waste generated, waste recycled & waste dumped with breakup into categories like hazardous, plastic, e-waste, bio-medical waste etc. as per BRSR format:</label>
                        <table>
                            ...
                            <?php
                                ...
                                $limited_t_wasteDetails = array_slice($t_wasteDetails, 9, 13);
                            ...
                            ?>
                            <!--tr>
                                <th colspan="3">For each category of waste generated, total waste disposed by nature of disposal method (in metric tonnes)</th>
                            </tr>
                            <tr>
                                <th coslpan="3">Category of waste</th>
                            </tr-->
                            <!--?php
                                // Limit to the rows from index 13 to 17
                                $limited_t_wasteDetails = array_slice($t_wasteDetails, 13, 17);
                                $index = 13; // Start index from 14
                                foreach ($limited_t_wasteDetails as $rowData) {
                                    echo '<tr>';
                                         $index++; // Increment index for the next rowx
                                        if ($index == 14) {
                                            echo '<th>(i) Incineration</th>';
                                        } elseif ($index == 15) {
                                            echo '<th>(ii) Landfilling</th>';
                                        } elseif ($index == 16) {
                                            echo '<th>(iii) Other disposal operations</th>';
                                        } else {
                                            echo '<th>Total</th>';
                                        }
                                        // Fetch data starting from 9th column onwards
